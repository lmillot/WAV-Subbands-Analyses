# -*- coding: utf-8 -*-
"""
script:             tools_ids_profiles_plot_resynthesis.py

creation date:      13/10/2015, 16h10
modification date:  16/08/2024, 19h49
...
def plot_IDS_profile(ids_array, ids_mean_level, freq_array,
                     ...
                     the_format="jpg", the_fig_size=(8.33, 6), the_dpi=72,
                     I_close_fig=1):
...
# the_fig_size=(11, 8.5)
    ...
    if channels_number == 1:
        ax3.step(x_limits, y_ids_array, color="red", linewidth=3.5, 
                 linestyle="-")
    else:
        ax3.step(x_limits, y_ids_array[0], color="red", linewidth=3.5, 
                 linestyle="-")

        ax3.step(x_limits, y_ids_array[1], color="black", linewidth=3.5, 
                 linestyle="-")
    ...
    if the_format == "jpg":
        theFilename = the_radical + fig_name + ".jpg"
        plt.savefig(fig_path / theFilename, format="jpg", dpi="figure")
    else:
        theFilename = the_radical + fig_name + ".pdf"
        plt.savefig(fig_path / theFilename, format="pdf")
...
def main():
    ...
    freqs_array = np.array([0, 50, 200, 400, 800, 1200, 1800,
                           3000, 6000, 15000, 22050])

    min_ids_value = ids_array.min()
    max_ids_value = ids_array.max()

    plot_IDS_profile(ids_array, ids_mean_level, freqs_array,
                     channels_number, min_ids_value, max_ids_value,
                     fig_name, fig_path)
